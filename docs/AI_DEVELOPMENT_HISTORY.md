...
#### 프롬프트 1: 프로젝트 초기 설정
```
국가데이터처에서 발간하는 "지역경제동향" 분기별 보도자료를 자동으로 생성하는 
시스템을 만들고 싶어. 현재는 수작업으로 엑셀 데이터를 보도자료에 옮기는데 
...
#### 프롬프트 4: 정답 검증 시스템
```
correct_answer 폴더에 있는 정답 이미지(실제 보도자료 캡처)와 
생성된 보도자료의 수치가 완전히 일치하는지 검증하는 시스템을 만들어줘.
...
3. 보도자료 목록 표시 (좌측 사이드바)
4. 선택한 보도자료 미리보기 (우측 영역)
...
#### 프롬프트 7: 템플릿 모듈화
```
Jinja2 템플릿을 각 보도자료별로 분리해서 모듈화해줘.
...
#### 프롬프트 9: 스키마 기반 아키텍처
```
현재 각 보도자료마다 하드코딩된 셀 위치를 사용하고 있어.
...
이렇게 하면 새 보도자료 추가 시 스키마만 작성하면 돼.
```
...
- routes/
  ...
  - preview.py (미리보기)
- services/
  - report_generator.py (보도자료 생성 로직)
- utils/
...
업로드 즉시 자동으로 첫 번째 보도자료 미리보기를 표시해줘.

미리보기 버튼은 제거하고, 
좌측 목록에서 보도자료 클릭 시 바로 미리보기 갱신되도록 해줘.
```
