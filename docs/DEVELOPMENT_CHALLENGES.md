...
#### 문제 상황
국가데이터처에서 제공받은 분석표 엑셀 파일의 **시트명과 데이터 위치가 예상과 달랐습니다**.
...
12/30: 시도별 보고서 기초자료 시트 fallback 추가
```
...
2. **보고서별 상이한 키**: 광공업생산은 `manufacturing_data`, 서비스업은 `service_data` 등 패턴 불일치
...
### 9. 🔴 파일 업로드 과정에서 비어있는 보고서 생성 (원인 파악 지연)

#### 문제 상황
파일을 업로드했는데 **보고서가 비어있는 상태로 생성**되는 문제가 발생했습니다. 처음에는 템플릿 문제나 데이터 추출 로직 문제로 생각하고 해당 부분을 반복적으로 수정했으나, **실제 원인은 파일 업로드 프로세스 자체**에 있었습니다.

#### 증상
- 엑셀 파일 업로드 후 보고서 미리보기가 모두 빈 화면으로 표시
- 데이터가 전혀 추출되지 않거나 None으로 표시
- 일부 보고서만 정상 동작하고 나머지는 비어있음
...
2. **부분적 동작**: 일부 보고서는 캐시된 데이터로 정상 표시되어 혼란
...
| 🔴 **파일 업로드/경로** | 1건 | 파일 경로 불일치로 빈 보고서 생성 (원인 파악 지연) |
...
### 6. 🔴 기본부터 확인하기 (가장 중요한 교훈)
> **문제**: 비어있는 보고서 생성 시 복잡한 로직(템플릿, 데이터 추출)만 의심하고, **파일 자체가 제대로 업로드/저장되었는지** 기본적인 확인을 나중에 함
> 
> **개선**: 
...
1. **예상 외의 데이터 구조**: 제공받은 엑셀 파일의 구조가 문서와 다르고 분기별로 변동
2. **중간 아키텍처 변경**: 하드코딩 → 스키마 기반으로 전환하면서 대규모 수정
3. 🔴 **파일 업로드 버그 원인 파악 지연**: 비어있는 보고서가 생성되는 문제의 원인이 복잡한 로직이 아닌 **파일 경로 문제**였으나, 이를 뒤늦게 발견
4. **복잡한 요구사항 발견**: 단순 데이터 추출이 아닌 계산, 순위, 가중치 등 복잡한 로직 필요
5. **출력 포맷 시행착오**: HWPX 직접 생성에서 HTML 복붙 방식으로 전략 변경
6. **테스트 자동화 부재**: 수정 시 기존 기능 깨짐을 수동으로만 확인
