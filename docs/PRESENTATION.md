...
flowchart LR
    ...
    D -->|최종 보고서| E[HTML<br/>출력<br/>최종 보고서]
```
...
시스템은 **다층 캐싱 전략**을 통해 Excel 파일 읽기 및 데이터 처리 속도를 크게 향상시켰습니다. 캐싱 없이는 동일한 시트를 여러 Generator에서 반복적으로 읽어야 하며, 이는 전체 보고서 생성 시간을 크게 증가시킵니다.
...
**시나리오: 시도별 보고서 생성 (17개 시도 × 8개 지표 = 136개 보고서)**
...
**사례 1: 시도별 보고서 생성 (RegionalGenerator)**
...
| **총 메모리 사용** | **약 24-51MB** | 보고서 생성 중 일시적 |
...
| 시트 읽기 (평균 3시트/보고서) | 45 × 3 × 150ms = 20.3초 | 9시트 × 150ms = 1.35초 | **93% 감소** |
...
시스템은 **Chart.js**와 **HTML5 Canvas API**를 활용하여 인터랙티브한 차트와 인포그래픽을 동적으로 생성합니다. 보고서의 시각화 요소는 모두 클라이언트 사이드에서 렌더링되며, 사용자가 설정을 조정할 수 있는 기능을 제공합니다.
...
**용도:** 시도별 보고서에서 시계열 데이터 표시 (여러 시리즈, 점선/실선 구분)
...
```mermaid
flowchart TD
    A[POST /api/generate-all] --> B[모든 보고서 순차 생성]
    B --> C[각 HTML 파일 저장]
    C --> D[생성 결과 반환]
```
...
- 변환된 분석표는 다운로드 후 다시 업로드해야 보고서 생성 가능
...
이를 통해 분석 시트에 오류가 있어도 보고서가 정상적으로 생성됩니다.
...
| **가중치 설정** | `#weightInfoBtn` | 보고서 생성 시 사용할 가중치 값을 미리 설정 (선택사항) |
...
사이드바는 왼쪽에 위치하며, 데이터 입력과 보고서 목록 탐색 기능을 제공합니다.
...
**B. 보고서 목록 섹션** (`#reportListSection`)

| 구성 요소 | 기능 | 상태 표시 |
|----------|------|----------|
| **전체 보고서 통합 리스트** | 요약/부문별/시도별/통계표를 하나의 리스트로 통합 | 섹션 헤더로 구분 (📋 요약, 📊 부문별, 🗺️ 시도별, 📈 통계표) |
| **보고서 아이템** | 각 보고서를 클릭하여 미리보기 표시 | ○ 미생성<br>● 생성됨<br>✓ 검토 완료<br>! 결측치 있음<br>✏️ 편집됨 |
| **아이콘 표시** | 각 보고서 타입을 구분하는 이모지 아이콘 | 보고서별 고유 아이콘 (🏭 광공업생산, 🏢 서비스업생산 등) |
...
메인 콘텐츠 영역은 보고서 미리보기와 작업 제어 기능을 제공합니다.
...
| 구성 요소 | ID | 기능 |
|----------|-----|------|
| **이전/다음 버튼** | `#prevBtn`, `#nextBtn` | 보고서 간 순차적 이동 (이전/다음 페이지) |
| **페이지 인디케이터** | `#pageIndicator` | 현재 보고서 위치 표시 (예: "15 / 50") |
| **HTML 보도자료 다운로드** | `#exportBtn` | 모든 보고서를 하나의 HTML 파일로 다운로드 (PDF 인쇄용) |
...
| 구성 요소 | ID | 기능 |
|----------|-----|------|
| **미리보기 제목** | `#previewTitle` | 현재 보고서 이름 표시 |
| **편집 버튼** | `#editBtn` | 미리보기를 편집 가능한 모드로 전환 |
| **저장/취소 버튼** | `#saveEditBtn`, `#cancelEditBtn` | 편집 내용 저장 또는 취소 |
| **검토 완료 버튼** | `#markReviewedBtn` | 현재 보고서를 검토 완료 상태로 표시 |
| **플레이스홀더** | `#previewPlaceholder` | 보고서 선택 전 안내 메시지 |
| **iframe 미리보기** | `#previewIframe` | HTML 보고서를 iframe으로 렌더링 |
...
| **현재 항목** | `#loadingCurrentIcon`, `#loadingCurrentText` | 현재 생성 중인 보고서 이름 표시 |
| **경과 시간** | `#elapsedTime` | 보고서 생성 시작 후 경과된 시간 (MM:SS 형식) |
...
- 표지 보고서 미리보기 자동 새로고침
- 모든 요약 보고서에 담당자 정보 반영

**B. 가중치 설정 모달** (`#weightSettingsModal`)

| 기능 | 설명 |
|------|------|
| **가중치 값 입력** | 보고서 생성 시 사용할 가중치 값 미리 설정 (선택사항) |
...
| 기능 | 설명 |
|------|------|
| **결측치 자동 감지** | 보고서 생성 시 필수 데이터가 누락된 경우 자동으로 모달 표시 |
| **동적 입력 필드 생성** | 누락된 필드별로 입력 필드 자동 생성 |
| **저장/건너뛰기** | 입력 값을 저장하거나 건너뛰기 선택 가능 |
| **저장된 값 활용** | 저장된 값은 해당 보고서 생성 시 자동 적용 |
...
| 타입 | 색상 | 사용 시기 |
|------|------|----------|
| **success** | 초록색 | 작업 성공 시 (예: 파일 업로드 완료, 보고서 생성 완료) |
...
> 시스템은 이 **검증된 데이터를 신뢰**하고, **보고서 생성 자동화에 집중**합니다."
...
**현재 검증 방법:**
1. **수동 비교 검증**: 생성된 보고서와 `correct_answer/` 폴더의 정답 이미지 육안 비교
2. **미리보기 시스템**: 담당자가 각 보고서를 하나씩 검토 후 승인
...
#### Q8. 분석표의 데이터에 오류가 있으면 보고서도 틀린 내용이 나오는데, 이걸 어떻게 방지합니까?
...
**담당 부서 설득 포인트:**
- "Python 설치"가 아닌 "보고서 작성 도구 파일 1개"
- 오프라인 동작 → 정보 유출 위험 없음
- 언제든 삭제 가능 → 위험 최소화
...
```python
# DEBUG 모드에서 상세 로그 출력
print(f"[DEBUG] ========== {report_name} 보고서 생성 시작 ==========")
...
# 2. 단일 실행 파일 빌드
pyinstaller --onefile --noconsole --name "지역경제동향_보고서생성기" app.py \
...
3. **바탕화면 바로가기 확인**
   - "보고서생성" 아이콘이 생성되었는지 확인

### 일상 사용 절차

1. **프로그램 시작**
   - 바탕화면의 "보고서생성" 아이콘 더블클릭
   - 웹 브라우저 자동 실행 (약 5초)
...
- **보고서생성.bat**: 가상환경 활성화, 중복 실행 감지, 브라우저 자동 실행, Flask 서버 시작
