...
- **시간**: 2026-01-16
- **요청 사항**:
  - 실무자가 한글 문서에 복붙할 표와 나레이션만 포함하도록 보도자료 축소
  - 포함: 요약(5개), 부문별(10개), 시도별(17개) - 표 + 나레이션만
  - 제외: 표지, 목차, 일러두기, 인포그래픽, 차트/그래프, 통계표, GRDP
- **원인 분석**:
  - 고객사 요구사항 변경: 실무자는 표와 텍스트만 복붙하여 사용
  - 기존 시스템은 표지, 목차, 차트 등 모든 요소를 포함하여 생성
- **에이전트 사고 과정**:
  - 문제 인식: 사용자가 "표와 나레이션만 전체 붙여넣기할 것"이라고 명시
  - 분석:
    1. 현재 HTML 생성 엔드포인트 `/export-hwp-ready`에서 일부 섹션만 제외 중
    2. 템플릿에 차트 요소가 있는지 확인 필요
    3. 제외 리스트에 추가 섹션(일러두기, 인포그래픽, 통계표, GRDP) 추가 필요
  - 고려한 해결책:
    1. HTML 생성 단계에서 제외: `excluded_report_ids`에 추가 (선택)
    2. 템플릿에서 차트 제거: canvas/svg 요소 삭제
    3. 설정 파일 주석 업데이트
  - 선택한 접근 방법:
    - 방법 1 선택: 가장 간단하고 명확한 방법
    - 템플릿 검증: 이미 차트 요소가 없음을 확인
- **해결 방법**:
  1. `routes/api.py`의 `/export-hwp-ready` 엔드포인트에서 `excluded_report_ids` 업데이트
     - 기존: `{'cover', 'toc', 'stat_toc'}`
     - 변경: `{'cover', 'toc', 'stat_toc', 'guide', 'infographic', 'stat_appendix', 'stat_grdp'}`
  2. `config/reports.py`에 주석 업데이트
     - SUMMARY_REPORTS 주석에 제외 항목 명시
     - STATISTICS_REPORTS 주석 보완
     - PAGE_CONFIG에 사용 중단 명시
  3. 템플릿 검증
     - 요약 템플릿 5개: canvas/Chart.js 없음 확인
     - 부문별 템플릿 10개: canvas/Chart.js 없음 확인
     - 시도별 템플릿: canvas/Chart.js 없음 확인
- **관련 파일**:
  - `routes/api.py` (라인 1703-1704)
  - `config/reports.py` (라인 7, 155, 178)
- **상태**: 완료
- **참고 사항**:
  - 템플릿에는 이미 차트 요소가 없어 추가 작업 불필요
  - 한글 복붙용 HTML 생성 시 표지, 목차, 일러두기, 인포그래픽, 통계표, GRDP가 자동으로 제외됨
  - 생성되는 문서는 순수하게 표와 나레이션만 포함
