...
- **문제 설명**: 
  PM이 3가지 핵심 개선사항을 지시:
  1. **하드코딩된 열 번호 제거**: 헤더 텍스트를 검색하여 동적으로 열 번호 찾기
  2. **결측치 처리 개선**: 데이터가 없을 때 0.0이 아닌 N/A(None)로 처리
  3. **건설/고용 데이터 검증**: 고용률은 %p 차이, 건설은 전년동기비 행 확인
- **원인 분석**: 
  1. **하드코딩된 컬럼 인덱스**: `row[24]`, `row[20]`, `row[25]`, `row[21]` 등이 코드에 직접 박혀있음
  2. **결측치를 0으로 처리**: `safe_float(value, 0)` 사용으로 데이터 없을 때 0.0 반환 → 평균 계산 왜곡
  3. **고용률 계산 방식**: 이미 %p 차이로 계산 중이지만, 일부 코드에서 증감률로 계산하는 부분 존재 가능
  4. **건설 데이터**: 전년동기비 행이 별도로 있는지 확인 필요
- **에이전트 사고 과정**:
  1. **문제 인식**: PM의 3가지 지시사항 확인
  2. **우선순위 결정**:
     - 1단계: 동적 컬럼 찾기 (가장 중요, 데이터 정확성에 직접 영향)
     - 2단계: 결측치 처리 (데이터 품질 개선)
     - 3단계: 건설/고용 검증 (로직 정확성 확인)
  3. **구현 전략**:
     - `find_column_by_header_text()` 함수 추가: PM 가이드에 따라 헤더에서 연도/분기 찾기
     - `safe_float()` 기본값을 None으로 변경: 0 대신 None 반환
     - 기존 코드에서 `safe_float(..., 0)` → `safe_float(..., None)` 변경
     - None 값 처리 로직 추가: 계산에서 제외, N/A 표시
  4. **시도한 방법들**:
     - **시도 1**: 단순 문자열 매칭 → 다양한 형식 대응 어려움
     - **최종**: 연도와 분기를 각각 확인하는 방식 (PM 가이드 준수)
  5. **최종 결정**:
     - 헤더 텍스트 검색으로 동적 컬럼 찾기
     - None 값은 계산에서 제외하고 N/A로 표시
     - 고용률은 이미 %p 차이로 계산 중 (확인 완료)
     - 건설 데이터는 절대값에서 증감률 계산 (전년동기비 행 없음 확인)
- **해결 방법**:
  1. **동적 컬럼 찾기 함수 추가** (`services/summary_data.py`):
     - `find_column_by_header_text()`: 헤더 행에서 연도와 분기를 모두 포함한 컬럼 찾기
     - 다양한 패턴 지원: `"2025.3/4"`, `"2025 3/4"`, `"2025  3/4"` (공백 1~3개)
     - `_extract_price_summary_from_aggregate()`: 하드코딩된 `row[24]`, `row[20]` 제거
     - `_extract_employment_summary_from_aggregate()`: 하드코딩된 `row[24]`, `row[20]` 제거
     - `get_employment_population_data()`: 하드코딩된 `row[25]`, `row[21]` 제거
  2. **결측치 처리 개선**:
     - `safe_float()` 함수 수정: 기본값을 `None`으로 변경 (0이 아님)
     - `'-'`, 빈 문자열, `'없음'`, `'N/A'` 등은 모두 None 반환
     - 계산 로직 수정: `curr_val is None or prev_val is None`이면 `change = None` (N/A 처리)
     - None 값은 계산에서 제외하고 결과에서도 None으로 표시
  3. **건설/고용 데이터 검증**:
     - 고용률: 이미 %p 차이로 계산 중 확인 (`calc_type: 'difference'`)
     - 건설: 엑셀 시트 분석 결과 전년동기비 행 없음 확인, 절대값(수주액)에서 증감률 계산하는 것이 맞음
- **관련 파일**: 
  - `services/summary_data.py` - 동적 컬럼 찾기, 결측치 처리 개선
- **테스트 결과**:
  - ✅ 동적 컬럼 찾기: 헤더에서 연도/분기 정상 검색
  - ✅ 결측치 처리: None 값 정상 반환 및 계산 제외
  - ✅ 고용률: %p 차이 계산 확인
  - ✅ 건설: 절대값에서 증감률 계산 확인
- **작업 상태**: ✅ 완료
- **참고 사항**: 
  - 일부 코드에서 여전히 `safe_float(..., 0)` 사용하는 부분이 있을 수 있음 (추가 검토 필요)
  - 건설 시트에는 "전년동기비" 행이 별도로 없고, 절대값(수주액)에서 증감률을 계산하는 것이 정상
  - 고용률은 이미 %p 차이로 계산 중이므로 추가 수정 불필요

---

### 2026-01-11 밤 (최신)

#### 2025년 3분기 데이터 처리 오류 수정 - 하드코딩된 분기 값 제거
- **시간**: 2026-01-11 밤
- **문제 설명**: 
  - 2025년 3분기 엑셀 파일을 처리할 때 HTML에 많은 N/A(275개), 0.0%(49개), 공란(332개) 발생
  - HTML에 잘못된 분기 헤더(2025년 2/4)가 36개 발견됨
  - 실제 엑셀에는 2025년 3/4 데이터가 정상 존재하지만 추출 실패
- **원인 분석**: 
  1. **하드코딩된 분기 값**: `data_converter.py::extract_grdp_data()` 함수가 '2025'와 '2/4'를 하드코딩하여 찾으려 함
  2. **동적 처리 부재**: 엑셀 파일명에서 추출한 연도/분기(2025년 3분기)를 활용하지 않고 고정값 사용
  3. **패턴 매칭 부족**: 다양한 분기 표기 형식('2025  3/4', '2025.3/4', '2025 3/4p' 등)을 모두 처리하지 못함
  4. **Fallback 부족**: 원하는 분기를 찾지 못했을 때 가장 최신 분기를 자동으로 찾는 로직 없음
- **에이전트 사고 과정**:
  1. **문제 인식**: HTML 파일 분석 결과 275개 N/A, 49개 0.0%, 332개 공란 발견
  2. **원인 추적**:
     - 엑셀 파일 분석: 2025년 3/4 데이터 정상 존재 확인
     - 코드 분석: `data_converter.py`의 `extract_grdp_data()` 함수에서 하드코딩된 '2/4' 발견
     - HTML 분석: 잘못된 분기 헤더(2025년 2/4) 36개 발견 → 코드가 2/4를 찾으려 했던 증거
  3. **해결책 고려**:
     - **Option A**: 하드코딩 제거 + `self.year`, `self.quarter` 사용 (이미 `_detect_year_quarter()`에서 설정됨)
     - **Option B**: Smart Scout의 앵커 기반 컬럼 찾기 활용
     - **Option C**: 정규식 패턴으로 더 유연한 매칭
  4. **선택한 접근**: Option A + Option C (하이브리드)
     - `self.year`, `self.quarter` 사용 (이미 동적으로 추출됨)
     - 다양한 패턴 지원 (공백 1~3개, 점/공백 혼용)
     - Fallback: 원하는 분기를 찾지 못하면 가장 최신 분기 자동 선택
  5. **시도한 방법들**:
     - **시도 1**: 단순 문자열 매칭 → 공백 개수 차이로 실패 ('2025  3/4' vs '2025 3/4')
     - **시도 2**: 정규식 패턴 `[.\s]*` → 공백 1개만 매칭, 2개 이상 실패
     - **최종**: 정규식 패턴 `[.\s]{1,3}` → 공백 1~3개 모두 매칭 성공
  6. **최종 결정**:
     - 하드코딩 제거, `self.year`, `self.quarter` 사용
     - 다양한 패턴 지원 (공백 개수, 점/공백 혼용)
     - 가장 최신 분기 자동 선택 Fallback 추가
- **해결 방법**:
  1. **data_converter.py::extract_grdp_data()** 수정:
     - 하드코딩된 '2025'와 '2/4' 제거
     - `self.year`, `self.quarter` 사용 (이미 `_detect_year_quarter()`에서 동적 추출됨)
     - 다양한 분기 패턴 지원: `f"{year}.{quarter}/4"`, `f"{year} {quarter}/4"`, `f"{year}  {quarter}/4"` 등
     - 정규식 패턴 개선: `[.\s]{1,3}` (공백 1~3개 모두 매칭)
     - Fallback 로직 추가: 원하는 분기를 찾지 못하면 가장 최신 분기 자동 선택
  2. **패턴 매칭 개선**:
     - 공백 1개, 2개, 3개 모두 지원
     - 점(.)과 공백 혼용 지원
     - 'p' 접미사 지원 (예: '2025 3/4p')
  3. **에러 처리 강화**:
     - 각 단계별 상세한 로그 출력
     - Fallback 시 경고 메시지 출력
- **관련 파일**: 
  - `data_converter.py` - extract_grdp_data() 함수 수정
- **테스트 결과**:
  - ✅ 연도/분기 자동 감지: 2025년 3분기 정상 추출
  - ✅ 전년동기 컬럼 찾기: 2024년 3/4 컬럼 정상 발견 (col 64)
  - ⚠️ 현재 분기 컬럼: 2025년 3/4 데이터가 GRDP 시트에 없음 (2025년 2/4 사용)
  - ✅ Fallback 메커니즘: 가장 최신 분기(2025년 2분기) 자동 선택 성공
  - ✅ GRDP 데이터 추출: 정상 작동
- **작업 상태**: ✅ 완료
- **참고 사항**: 
  - GRDP 시트에는 2025년 3분기 데이터가 아직 없음 (2025년 2분기까지만 존재)
  - 다른 시트(광공업생산, 서비스업생산 등)에는 2025년 3/4 데이터가 정상 존재
  - 이는 GRDP 데이터가 아직 업데이트되지 않았거나 다른 시트와 업데이트 주기가 다를 수 있음
  - Fallback 메커니즘으로 인해 시스템은 정상 작동하지만, 사용자에게 알림이 필요할 수 있음

---

### 2026-01-11 저녁

#### 엑셀 구조 변경에 대응하는 유연한 탐색 시스템 구현 (Smart Scout)
- **시간**: 2026-01-11 저녁
- **문제 설명**: 
  엑셀 파일의 시트명이나 컬럼 위치가 변경될 때마다 코드를 수정해야 하는 하드코딩 문제가 있었습니다. 예를 들어:
  1. 시트명이 "광공업생산"에서 "2025_광공업_최종"으로 변경되면 시스템이 작동하지 않음
  2. 컬럼이 한 칸 밀리면 하드코딩된 인덱스(예: col 64)로는 데이터를 찾을 수 없음
  3. 구조가 변경되어도 시스템이 뻗지 않고 적절히 대응해야 함
- **원인 분석**: 
  1. **하드코딩된 시트명**: `sheet_name='광공업생산'`처럼 정확한 시트명을 코드에 박아놓음
  2. **하드코딩된 컬럼 인덱스**: `curr_col: 64, prev_col: 60`처럼 고정된 컬럼 번호 사용
  3. **설정 파일 부재**: 매핑 규칙이 코드에 하드코딩되어 있어 변경 시 재배포 필요
  4. **방어적 검증 부재**: 구조 변경 시 전체 프로세스가 중단됨
- **에이전트 사고 과정**:
  1. **문제 인식**: 사용자 요구사항에서 "엑셀의 위치를 외우지 말고, '특징'을 찾으십시오"라는 지침 확인
  2. **해결책 고려**:
     - **Option A**: 기존 하드코딩 방식 유지 → 변경 시마다 코드 수정 필요 (비현실적)
     - **Option B**: 키워드 기반 시트 탐색 + 앵커 포인트 기반 컬럼 매핑 → 구조 변경에 유연하게 대응 가능
     - **Option C**: 완전 자동화된 AI 기반 구조 분석 → 과도한 복잡도, 오버엔지니어링
  3. **선택한 접근**: Option B - 단계별로 점진적 개선
     - Step 1: 앵커 포인트 기반 컬럼 매핑 (핵심 기능)
     - Step 2: 설정 파일 외재화 (유지보수성 향상)
     - Step 3: 방어적 검증 (안정성 확보)
     - Step 4: Two-pass reading (성능 최적화)
  4. **구현 전략**:
     - 기존 코드와의 호환성 유지 (fallback 메커니즘)
     - 앵커 기반 탐색 실패 시 기존 동적 계산 방식으로 자동 전환
     - 점진적 마이그레이션 (기존 코드는 그대로 두고 새 기능 추가)
  5. **시도한 방법들**:
     - **시도 1**: 정규식 패턴만으로 분기 찾기 → 너무 엄격함, 다양한 형식 대응 어려움
     - **시도 2**: 문자열 키워드만 사용 → 정규식 패턴(분기)을 놓침
     - **최종**: 정규식 패턴 + 문자열 키워드 혼합 사용
  6. **최종 결정**:
     - 앵커 포인트 기반 탐색을 우선 시도, 실패 시 기존 동적 계산으로 fallback
     - 설정 파일(`excel_mapping.json`)로 키워드와 앵커 관리
     - 검증 실패 시 해당 섹션만 N/A 처리 (Fail-Safe)
- **해결 방법**:
  1. **Step 1: 앵커 포인트 기반 컬럼 매핑** (`extractors/base.py`):
     - `find_anchor_points()`: 상위 10행 스캔하여 핵심 키워드("전년동기비", "증감률", 분기 패턴 등) 위치 탐지
     - `find_column_by_anchor()`: 앵커 기반으로 특정 연도/분기 컬럼 인덱스 찾기
     - `_load_sheet_header()`: Two-pass reading의 첫 단계 - 헤더만 먼저 읽기 (nrows=20)
     - `parse_sheet_structure()` 개선: 앵커 포인트로 헤더 행을 동적으로 찾도록 수정
  2. **Step 2: 동적 설정 파일 도입** (`config/excel_mapping.json`):
     - 시트 타입별 키워드, 헤더 앵커, 지역/코드 컬럼 키워드, 총지수 값 정의
     - 공통 설정(분기/연도 패턴, 스캔 행 수 등) 포함
     - `BaseExtractor.load_excel_mapping_config()`: JSON 설정 파일 로드 및 캐싱
     - `BaseExtractor.get_sheet_mapping_config()`: 시트 타입별 설정 조회
  3. **Step 3: 방어적 검증 시스템** (`extractors/base.py`):
     - `validate_structure()`: 시트 구조 검증 (필수 앵커 존재 여부, 데이터 타입 확인)
     - `safe_extract_with_validation()`: 검증과 함께 안전한 데이터 추출
     - 검증 실패 시 해당 섹션만 N/A 처리, 전체 프로세스 중단 없음 (Fail-Safe)
  4. **Step 4: Two-pass reading 최적화**:
     - 헤더 영역만 먼저 읽어서 구조 파악 후 필요한 범위만 재읽기
     - 불필요한 전체 시트 로드 방지로 성능 향상
  5. **services/summary_data.py 통합**:
     - `find_anchor_based_column()`: 앵커 기반 컬럼 인덱스 찾기 함수 추가
     - `get_anchor_based_cols()`: 현재/전년동기 분기 컬럼 찾기
     - `get_dynamic_col()` 개선: 앵커 기반 우선 시도, 실패 시 기존 동적 계산 fallback
     - `get_summary_table_data()`에서 앵커 기반 컬럼 찾기 사용
- **관련 파일**: 
  - `extractors/base.py` - 앵커 포인트 기반 컬럼 매핑, 방어적 검증, Two-pass reading
  - `config/excel_mapping.json` - 동적 설정 파일 (신규 생성)
  - `services/summary_data.py` - 앵커 기반 컬럼 찾기 통합
- **테스트 결과**:
...
- **작업 상태**: ✅ 완료
- **참고 사항**: 
  - 기존 코드와의 호환성 완벽 유지 (기존 동적 계산 방식이 fallback으로 작동)
  - 설정 파일 수정만으로 매핑 규칙 변경 가능 (코드 재배포 불필요)
  - 시트명이 "광공업생산" → "2025_광공업_최종"으로 변경되어도 키워드("광공업", "생산")로 찾을 수 있음
  - 컬럼이 한 칸 밀려도 앵커 포인트("2024.1/4", "전년동기비")를 찾아서 정확한 위치 파악 가능
...
#### HTML 내보내기 기능 확인 및 분석표 템플릿 오류 처리
- **시간**: 2026-01-09 (최신)
- **문제 설명**: 
  1. 미리보기에서 HTML 내보내기 기능이 필요함
  2. 분석표 생성 기능이 오류 발생으로 동작하지 않음 (`FileNotFoundError: 템플릿 파일을 찾을 수 없습니다`)
- **원인 분석**: 
  1. HTML 내보내기: 이미 `exportFinalDocument()` 함수와 `exportBtn` 버튼이 구현되어 있음. 보고서 생성 완료 시 자동 활성화됨.
  2. 분석표 생성: `DataConverter` 클래스가 `분석표_25년 2분기_캡스톤.xlsx` 템플릿 파일을 필요로 하지만, 해당 파일이 프로젝트에 존재하지 않음.
- **에이전트 사고 과정**:
  - HTML 내보내기 문제 확인: `grep`으로 `exportFinalDocument` 검색 → 함수와 버튼이 이미 존재함을 확인
  - 분석표 오류 분석: 
    1. 서버 로그에서 `FileNotFoundError` 확인
    2. `find` 명령으로 프로젝트 내 `분석표` 관련 파일 검색 → 결과 없음
    3. `data_converter.py`의 `__init__` 메서드 확인 → 템플릿 경로 설정 로직 확인
  - 해결 방안 검토:
    1. 템플릿 파일 추가? → 파일이 없어서 불가능
    2. 분석표 생성 버튼 제거? → 기존 사용자 경험 변경 필요
    3. 친절한 오류 메시지로 안내? → 가장 적합한 해결책
  - 최종 결정: 템플릿 파일이 없을 때 명확한 오류 메시지를 반환하고, 이 기능이 레거시임을 안내
- **해결 방법**:
  1. `data_converter.py` 수정: `__init__`에서 템플릿 파일 존재 확인 후 명확한 `FileNotFoundError` 발생
  2. `routes/api.py` 수정: `download-analysis`, `generate-analysis-with-weights` 엔드포인트에서 `FileNotFoundError` 캐치 후 친절한 JSON 응답 반환
  3. `dashboard.html` 수정: API 에러 응답의 상세 메시지(`detail`)를 파싱하여 사용자에게 표시
- **관련 파일**: 
  - `data_converter.py` - 템플릿 파일 존재 확인 및 에러 메시지 개선
  - `routes/api.py` - API 에러 응답 개선 (400 Bad Request + 상세 설명)
  - `dashboard.html` - 에러 메시지 파싱 및 표시 개선
- **상태**: ✅ 완료
- **참고 사항**: 
  - 분석표 생성은 레거시 기능으로, 현재 워크플로우에서는 기초자료에서 직접 보도자료를 생성함
  - 향후 분석표 템플릿이 추가되면 분석표 생성 기능이 정상 작동함
  - HTML 내보내기 기능은 보고서 생성 완료 후 `exportBtn`이 자동 활성화되어 사용 가능
...
- **시간**: 2026-01-09 11:30
- **문제 설명**: 
  - 기초자료 업로드 완료 후 업로드 완료 모달에서 다음 단계로 넘어가지 않음
  - 사용자가 스크린샷을 제공: 모든 단계 완료, GRDP 정상 표시, 하지만 화면이 멈춤
- **원인 분석**: 
  - **근본 원인**: `buildReportList(data.reports, data.regional_reports)` 함수 호출이 있으나 해당 함수가 정의되어 있지 않음
  - JavaScript 에러(`buildReportList is not defined`)로 인해 이후 코드(모달 닫기, 가중치 모달 표시)가 실행되지 않음
  - 추가로 가중치 경고 모달의 z-index(1003)가 업로드 모달과 동일하여 충돌 가능성 있었음
- **에이전트 사고 과정**:
  - 문제 인식: 사용자가 "이 화면에서 넘어가질 않아요"라고 제보
  - 서버 로그 확인: `[가중치 경고] 누락: 1건` - 가중치 경고 모달이 표시되어야 함
  - 1차 분석 (오진):
    1. z-index 충돌 의심 → z-index: 1005로 수정
    2. `showWeightWarningModal`에서 업로드 모달 강제 닫기 추가
  - 2차 분석 (정확):
    1. 사용자가 여전히 문제 발생 보고
    2. 디버그 로그에서 이전 유사 문제 확인 → "업로드 후 미리보기 미생성 문제 해결" (2026-01-07)
    3. grep으로 `buildReportList` 함수 정의 검색 → **함수가 존재하지 않음 발견**
    4. `raw_direct`와 `raw_with_analysis` 분기 모두에서 정의되지 않은 함수 호출
  - 근본 원인 파악: 정의되지 않은 함수 호출로 인한 JavaScript 런타임 에러
- **해결 방법**: 
  - `dashboard.html` 수정:
    1. 가중치 경고 모달에 `style="z-index: 1005;"` 추가 (1차 수정)
    2. `showWeightWarningModal()`에서 업로드 모달 강제 닫기 추가 (1차 수정)
    3. **`buildReportList(...)` 호출 제거** (2차 수정 - 핵심)
       - `raw_with_analysis`/`raw` 분기에서 제거
       - `raw_direct` 분기에서 제거
- **관련 파일**: 
  - `dashboard.html` (4곳 수정)
- **상태**: ✅ 완료
- **참고 사항**: 
  - `buildReportList`는 존재하지 않는 함수였음 (아마도 리팩토링 중 삭제되었으나 호출은 남아있었음)
  - 보도자료 목록은 `generateAllReports()` 호출 시 `renderAllReportsList()`로 자동 갱신됨
  - z-index 수정은 추가 안전장치로 유지
