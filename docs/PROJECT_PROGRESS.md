...
| 12/25 | 지역경제동향 보도자료 자동 생성 시스템 구현 | `992e94b` |
...
```mermaid
graph TB
    subgraph Flask[Flask 웹 애플리케이션 app.py]
        ...
        subgraph Templates[templates/]
            T1[*_template.html<br/>71개 HTML 템플릿]
            T2[*_generator.py<br/>15개 생성기]
            T3[*_schema.json<br/>39개 스키마]
        end
    end
...
└── templates/      # HTML 템플릿 및 생성기
    ├── *_template.html   # HTML 서식
    ├── *_generator.py    # 데이터 추출 (일부 하드코딩)
...
| 구분 | 스키마 (JSON 수정) | 하드코딩 (Python 수정) |
|------|-------------------|----------------------|
| 데이터 구조 정의 | ✅ `*_schema.json` | - |
| 업종/지역 매핑 | ✅ 스키마 내 정의 | ⚠️ Generator에도 중복 정의 |
| 텍스트 생성 규칙 | ✅ 스키마 내 정의 | - |
...
graph TB
    ...
    subgraph Main[메인 영역]
        ...
        subgraph Preview[미리보기 영역]
            PreviewArea[미 리 보 기 영 역]
            Nav[◀ 이전 다음 ▶]
        end
    end
...
| 🟡 중간 | 버전 관리 | 보도자료 버전 히스토리 관리 |
| 🟢 낮음 | 다국어 지원 | 영문 보도자료 생성 |
...
- [정답 이미지](./correct_answer/) - 목표 보도자료 이미지
