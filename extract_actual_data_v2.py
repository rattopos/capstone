import sys
def get_table_template_mapping():
    """
    data_table_locations.md에 정의된 표이름-템플릿 매핑 반환
    """
    mapping = {}
    for name, info in TABLE_LOCATIONS.items():
...
def save_table_html(table_name: str, output_path: str = None) -> str:
    """
    표이름에 해당하는 범위만 추출하여 HTML로 저장
    """
    # extract_table_by_location 함수가 반드시 위에 정의되어 있어야 함
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="정의된 표이름만 추출하여 HTML로 저장")
...
def extract_table_by_location(table_name: str) -> pd.DataFrame | None:
    """
    data_table_locations.md에 정의된 시트/범위만 추출
    """
