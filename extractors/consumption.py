...
class ConsumptionConstructionExtractor(BaseExtractor):
    ...
    def extract_consumption_data(self) -> Dict[str, Any]:
        ...
        # 전국 level=1 업태 추출
        national_businesses = self._extract_level_businesses(sheet_name, '전국', '소비', level=1)
    ...
    def extract_construction_data(self) -> Dict[str, Any]:
        ...
        # 전국 level=1 공종 추출
        national_sectors = self._extract_level_businesses(sheet_name, '전국', '건설', level=1)
        report_data['nationwide_data'] = {
            'order_amount': None,
            'growth_rate': national_rate,
    ...
    def _process_regional_data(self, current_data: Dict, sheet_name: str = None, category: str = None) -> List[Dict]:
        ...
        for region in ALL_REGIONS:
            ...
            if sheet_name and category:
                top_businesses = self._extract_level_businesses(sheet_name, region, category, level=2)
    ...
    def _extract_level_businesses(self, sheet_name: str, region: str, category: str, level: int = 2) -> List[Dict]:
