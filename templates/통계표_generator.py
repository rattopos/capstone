...
class 통계표Generator:
    ...
    def __init__(self, excel_path: str, historical_data_path: Optional[str] = None):
        ...
        Args:
            ...
            historical_data_path: 과거 데이터 JSON 파일 경로 (2017-2020 데이터)
        """
        ...
        for quarter_key, col_key in quarterly_mapping.items():
            ...
            for region in self.ALL_REGIONS:
                if col_key:
                    ...
                    if quarter_key in hist_data:
                        data["quarterly"][quarter_key][region] = hist_data[quarter_key].get(region)
            
            if data["quarterly"][quarter_key]:
                data["quarterly_keys"].append(quarter_key)
        
        return data
    
    def extract_all_tables(self, year: int = 2025, quarter: int = 2) -> Dict[str, Any]:
        """모든 통계표 데이터 추출"""
        tables = []
