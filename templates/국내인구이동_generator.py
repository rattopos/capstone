...
"""
국내인구이동 보고서 생성기
I(순인구이동)집계 및 I 참고 시트에서 데이터를 추출하여 HTML 보고서를 생성합니다.
"""
...
def load_data(excel_path):
    """엑셀 파일에서 데이터를 로드합니다."""
    summary_df = pd.read_excel(excel_path, sheet_name='I(순인구이동)집계', header=None)
    reference_df = pd.read_excel(excel_path, sheet_name='I 참고', header=None)
...
def main():
    ...
    template_path = os.path.join(base_dir, '국내인구이동_template.html')
    output_path = os.path.join(base_dir, '국내인구이동_output.html')
    json_output_path = os.path.join(base_dir, '국내인구이동_data.json')
