...
분석표를 기반으로 지역경제동향 보도자료를 자동 생성하는 웹 애플리케이션입니다.

## ✨ 주요 기능

| 기능 | 설명 |
|------|------|
| 📁 **분석표 업로드** | 드래그 앤 드롭 또는 클릭으로 분석표 엑셀 파일 업로드 |
| 📅 **연도/분기 자동 감지** | 엑셀 파일에서 연도와 분기를 자동으로 감지 |
| 🚀 **일괄 생성** | 모든 보도자료를 한 번에 자동 생성 |
| 📥 **한글 내보내기** | 한글(HWP) 복사용 HTML 파일 생성 |
| 🎨 **모던 UI** | Glassmorphism 디자인의 세련된 인터페이스 |
| 🔄 **동적 매핑** | 임의의 연도/분기에서도 작동하는 유연한 시스템 |
| ⚡ **통합 Generator** | 10개 부문 모두 단일 코드로 처리 |

## 🎯 기대 효과

### 시간 절감
- **보도자료 초안 작성**: 5일 → **약 3분** (99.9% 단축)
- 기초자료 수집표 업로드만으로 자동 생성
- 담당자는 한글 파일로 옮겨서 편집/검토만 수행

### 업무 효율화
- **해석 작업 시간 확보**: 2일 → **6일** (3배 증가)
- 단순 반복 작업에서 해방 → 분석 업무에 집중
- 더 깊고 넓은 해석과 통찰 가능
- **국민들에게 더 의미 있고 질 높은 분석과 전망** 제공

### 실무자 만족도 향상
- 복사-붙여넣기 및 수동 타이핑 작업 자동화
- 실수에 대한 긴장에서 벗어남
- 업무 피로도 및 시간 비효율성 해소

> **실무팀 구성**: 담당 사무관 1명, 주무관 1명 (총 2명)  
> **작업 기간**: 약 7일 → 효율 재분배로 해석 작업 시간 3배 확보
...
`app.py`의 `REPORT_ORDER` 배열 순서를 변경하면 보도자료 목록/다운로드 순서가 변경됩니다:
