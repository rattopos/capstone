...
def load_data(excel_path):
    ...
    for raw_sheet in raw_sheets:
        if raw_sheet in xl.sheet_names:
            ...
            use_raw_data = True
            print(f"[시트] 수집표 시트 사용: '{raw_sheet}'")
            break
...
def get_nationwide_data(df_analysis, df_index):
    ...
    # 조원 단위로 변환 (억원 -> 조원)
    construction_index_trillion = construction_index / 10000 if construction_index else 0.0
    
    # 전국 공종별 증감률 (분류단계 1인 행들)
    construction_types = []
    civil_growth = None  # 토목 증감률
    building_growth = None  # 건축 증감률
    
    for i in range(nationwide_idx + 1, min(nationwide_idx + 10, len(df_analysis))):
        row = df_analysis.iloc[i]
        ...
        # 토목/건축 증감률 저장
        if '토목' in type_name:
            civil_growth = type_growth
        elif '건축' in type_name:
            building_growth = type_growth
    ...
    return {
        ...
        'construction_index_trillion': round(construction_index_trillion, 1) if construction_index_trillion else 0.0,
...
def generate_report_data(excel_path, raw_excel_path=None, year=None, quarter=None):
    ...
    return {
        ...
        'increase_sub_types_text': increase_sub_types_text,
        'decrease_sub_types_text': decrease_sub_types_text,
        'summary_table': {
            'base_year': 2020,
            'columns': {
...
def generate_report(excel_path, template_path, output_path):
    ...
    # JSON 데이터 저장
    data_path = Path(output_path).parent / 'construction_data.json'
...
if __name__ == '__main__':
    ...
    template_path = Path(__file__).parent / 'construction_template.html'
    output_path = Path(__file__).parent / 'construction_output.html'
